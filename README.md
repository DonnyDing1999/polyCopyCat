...
| **health** |  | 在跟目标健康巡检（scout 排除规则的试用期考核）|
| `.check_interval_s` | 21600 | 巡检周期（默认 6 小时）；0 或 null 关闭 |
| `.auto_pause` | true | 目标命中排除规则（死仓暴增/频率突变/胜率崩等）自动暂停跟单 |
...
tests/                # 216 个单测，全部离线（HTTP/WS 均为注入的假实现）
...
版本号在 `pyproject.toml` 与 `polycopycat/__init__.py`（当前 0.21.0），每交付一个里程碑 minor +1。
