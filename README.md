...
- **排除**（宁严勿松）：样本不足、成交额太小、多日不活跃、回放亏损、胜率过低、频率过高（疑似机器人）、**大样本下胜率高得离谱（疑似结构性套利）**、**持仓浮亏占成本过高（疑似囤死仓/只认盈不认亏）**、**跨场馆套利单腿（胜率异常高 + 几乎全在贴近1.0平仓、几乎不割肉——输腿在别的账户/场外博彩，本钱包只见幸存赢腿）**、**慢速做市/流动性提供（同一 token 反复双向循环成交额占比高 + 双向点差薄——吃价差而非看方向；快进快出规则只抓分钟级快翻，这条抓小时级慢做市）**，**配对卖出不足（战绩无法自证）**——纯买入/撒币账户（如 500 笔全买入零卖出的撸空投地址）能把所有依赖卖出样本的规则整体绕过，一律不合格（仅招聘口径；在跟目标的巡检有窗口净盈亏规则兜底，不受此闸影响），**持仓中位不足 1 小时（速刷，0.32 起）**——票池规模化后实测：胜率 100%×19 笔、持仓中位半小时的速刷户恰好钻过所有大样本闸的门槛还拿满分，方向根本无法延迟复制，样本 ≥5 笔即排除（巡检同样适用，速刷化的在跟目标照停），**可跟性预检（0.34，仅发现/招聘口径）**——候选买入成交额中超过 70% 落在引擎 `skip_title_patterns` 过滤品类（比赛盘/日内盘）的直接不合格：0.33 实测招进 8 个高分目标全因品类不可跟空转（1,137 信号 0 执行），发现口径与执行过滤器从此共用一套真相；阈值放 70% 是刻意的——"一半比赛盘一半长线"的人仍然要（我们跟他长线那半），以及最关键的——**快进快出占比过高（疑似做市/套利）**。这类地址账面常年"盈利"、胜率漂亮，但赚的是价差返佣，方向毫无参考价值，跟单基本必亏
- **打分**（只在合格者间排序，0.32 起为 v2）：**ROI 30**（窗口盈亏/成交额，10% 拉满——比绝对盈亏公平，大资金不再天然高分）+ **Wilson 下界胜率 20**（95% 置信下界，小样本高胜率自动缩水）+ **可跟性 25**（持仓中位时长 15：半小时以下 0 分、24h 拉满，log 插值；极端价位买入占比 10：>0.85/<0.10 的仓没有延迟复制空间）+ **一致性 15**（最大单笔盈利占总盈利比，专杀"一把梭中了 longshot"的幸存者）+ **新鲜度 10**。原绝对盈亏/裸胜率/单笔规模/市场广度分全部退役。公式仍然刻意简单透明
- 窗口外建的仓（只见卖不见买）盈亏未知，不掺进胜率——只对能自证的部分下结论

### 常驻候选发现（run 引擎内置，0.31 起为票池 + 涓流评估）

`run` 起来后发现线程持续运转，不再是每日一次的快照突击（配置见 `health.discover_*`）：

- **票池三源**：实时流本来就收全站每一笔成交（客户端过滤），顺手把 wallet→成交额/笔数累积进账本 `discover_universe` 表（零 API 成本，约 10 分钟落库一次）；排行榜（7d/30d × 盈亏/成交额四组合，limit 逐档降级、挂了跳过）每个刷新周期并轨；全新账本用最近成交快照冷启动，不等周期
- **涓流评估**：每分钟从票池取 `discover_eval_per_min` 个地址走 scout 完整评估（回放+排除+打分），没评过的优先（按累积成交额降序）、评过的按 `discover_reeval_days` 冷却复评，结论写回票池行——评估进度和结论重启不丢
- **产出防刷屏**：合格且不在跟的地址进滚动名录 `discover-latest.json`（带完整评估结果与评估时间，每小时随摘要刷新）；通知每小时最多一条摘要、同一地址 7 天内不重复上榜；`auto_recruit` 打开才自动入池——**从名录全量按分数择优**（不是"这小时碰巧评出来的"），受 `recruit_min_score` 门槛与 `recruit_max_per_round` 限速约束，档位保守、黑名单与总数上限规则不变
...
| `force_exit_on_target_flat` | true | 对账兜底：发现「建仓目标已全部清仓、自己还持有」且连续两轮对账都成立时，按盘口强制平仓。防的是卖出信号丢失（超龄被拦/宕机漏听/双通道都漏）后拿着仓位到归零；非自己跟出来的仓（查不到建仓目标）不碰 |
...
| `.long_horizon_age_s` | 300 | 长线市场（距结束 ≥ long_horizon_days）放宽后的时效上限；null 关闭分级。**别调太低**：data-api `/trades` 从成交到可查中位滞后 ~160s、P90 ~283s，实时流没覆盖到的成交只能靠轮询捡，闸门低于这个数就是成批扔长线信号 |
| `.long_horizon_days` | 7 | 判定长线的最小剩余天数（市场结束时间取自 CLOB 元数据） |
| `.follow_sells` | true | 是否跟随卖出 |
| `.skip_title_patterns` | [] | 市场标题命中任一子串（不分大小写）即不跟，用于屏蔽短期指数/日内快盘（如 `["up or down", "opens up"]`）。**已持仓时的卖出信号不受此规则限制**（它只拦进场，不拦离场）|
| `.min_days_to_resolution` | null | **BUY 期限闸**：市场距结算不足 N 天不开新仓（卖出不受限；元数据缺结束时间的按短周期保守不跟）。纸面实测跟单亏损几乎全部来自比赛日盘/日内盘这类短周期市场——目标赚的是现场时机，延迟复制必然接刀，且没有中途纠错机会。null 关闭 |
| **risk** |  | 上限设为 null 表示不启用该项 |
| `.max_market_exposure_usdc` | 200 | 单市场持仓成本上限 |
| `.max_market_exposure_per_target_usdc` | null | **单目标单市场**净跟入成本上限（0.32 起）：防单个目标在同一市场反复加仓把账本压成他一个人的判断（纸面实测一个目标 5 笔加仓占掉 77% 仓位）。只拦 BUY，离场不受限；null 关闭 |
| `.exposure_groups` | [] | **事件族敞口闸**（0.34）：`[{"name","patterns":[关键词],"max_usdc"}]`——标题命中任一关键词的持仓合并计敞口，超上限拦新买。防"单市场闸各自不超、同一叙事一起输"（实证：六个伊朗局势相关仓合计 $205 压同一事件族）。刻意保守：同事件的反向腿也同计；自动事件聚类是后话 |
...
| `.discover_interval_s` | 86400 | 票池刷新周期：排行榜并轨（7d/30d × 盈亏/成交额）+ 修剪；0 或 null 关闭整个候选发现（见「常驻候选发现」）|
| `.discover_candidates` | 100 | 冷启动快照灌池的 top N（全新账本首次填充票池用）|
| `.discover_universe_size` | 5000 | 涓流评估的候选范围：票池按累积成交额降序前 N 名（池容量为其 4 倍，7 天不活跃修剪）|
| `.discover_eval_per_min` | 2.0 | 涓流速率：发现线程每分钟从票池取几个地址过 scout 完整评估；≤0 关涓流。默认速率下 5000 人票池约 1.7 天全覆盖一遍 |
| `.discover_reeval_days` | 3 | 复评冷却：评估过的地址隔几天才再评（没评过的永远优先）|
| `.auto_recruit` | false | 自动从**滚动名录**按分数从高到低招募进跟单池（**仅纸面模式**；招募档案 recruited.json 重启自动并回）。0.33 起取数源是名录全量而非"当小时刚评出的"——否则高分候选只因评估时间不对就永远轮不上 |
| `.recruit_min_score` | 70 | 招募分数门槛。实测名录合格者分数中位仅 ~45、前 10% 才 ~68，无门槛会招进大批擦线者 |
| `.recruit_max_per_round` | 3 | 每轮（每小时）最多招几个，分批进池便于观察；≤0 不限 |
| `.recruit_ratio` / `.recruit_max_per_trade_usdc` | 0.05 / 25 | 招募目标的保守档位（比常规目标小）|
| `.recruit_max_targets` | 15 | 目标总数上限（配置 + 招募），防无限膨胀 |
| `.recruit_dismiss_min_signals` | 50 | **零跟单率自动解聘**（0.34，仅对招募目标）：招满 `recruit_dismiss_min_hours`（默认 24）小时且有效信号 ≥ 此数、执行 0 条 → 解聘释放名额（有效信号剔除暂停期被自动拦下的——那不算给过机会）。实证：0.33 招进 8 个高分目标产生 1,137 条信号执行 0 条，全是当日体育盘。≤0 关闭；解聘后 `recruit_dismiss_cooldown_days`（默认 14）天内不回招。配置目标永不解聘（report 跟单率列给用户自己判断）|
| `.recruit_blocklist` | `[]` | 永不招募的地址：scout 分数看不出问题、但人工判定不该跟的（如只做当日盘，信号必然过期）。重启时也把已在招募档案里的黑名单地址剔出去 |
| **watch** |  | 信号源 |
...
**离场优先**（0.30 起）：已持仓时，该 token 的卖出信号**绕过时效闸和标题过滤**——你已经在场，「目标离场」这个事实本身就是信号，晚 2 分钟离场几乎总好过拿到归零（纸面实测过一笔：卖出信号超龄 144s 被拦，持有到结算 -100%）。兜底还有一道：对账发现建仓目标已全部清仓而自己还持有、且连续两轮成立，强制按盘口平仓（`force_exit_on_target_flat`），专防卖出信号彻底丢失的情况。
...
**`report --by-target`**：跟多个目标时，按目标拆分「已实现盈亏 / 结算归因 / 合计 / 累计买入 / 执行占信号比（跟单率）/ 各状态信号数」，用来回答"这几个人里谁的纸面跟单真赚钱、谁的动作根本跟不上"。卖出跟随平仓的盈亏经 orders→signals 干净归到目标；**市场结算（REDEEM）与对账强平的盈亏按该 token 各目标的建仓成本占比分摊归因**（0.30 起）——这正是纸面复盘里最大的一桶亏损，不归因就看不出谁在亏钱。查不到跟单建仓来源的仓位留在"未按目标归属"，可归因 + 未归属 = 账本总盈亏恒成立。
...
tests/                # 261 个单测，全部离线（HTTP/WS 均为注入的假实现）
...
版本号在 `pyproject.toml` 与 `polycopycat/__init__.py`（当前 0.29.0），每交付一个里程碑 minor +1。
...
- [x] 跟单止血包（0.30，源自纸面 10 天复盘：亏损全部来自短周期市场持有到归零）：BUY 期限闸 `min_days_to_resolution`、持仓卖出信号绕过时效闸/标题过滤（离场优先）、对账发现建仓目标全清仓即强制离场、`report --by-target` 结算盈亏按建仓成本归因到目标
- [x] 候选发现 v2（0.31）：票池三源（实时流被动累积 + 排行榜并轨 + 快照冷启动，sqlite 持久化）+ 每分钟涓流 scout 评估（5k 票池约 1.7 天全覆盖，冷却复评）+ 可判性闸（配对卖出不足不合格，堵纯买入撒币账户）+ 每小时摘要/7 天去重防刷屏
- [x] 跟单口径闭环（0.34，靶子是 0.33 招募的 8 个高分目标 1,137 条信号 0 执行的实证）：零跟单率自动解聘（招募目标专属，暂停期信号不计，解聘后冷却防回招）+ scout 可跟性预检（候选成交带过引擎标题过滤，>70% 不可跟品类即不合格——发现与执行共用一套真相）+ 事件族敞口闸 `risk.exposure_groups`（关键词族合并计敞口，防同一叙事的相关持仓一起归零）
- [x] 自动招募择优（0.33）：招募源从「当小时刚评出的合格者」改为**名录全量按分降序**（此前高分候选只因评估时间不对就永远轮不上）+ `recruit_min_score` 门槛 + 每轮限速
- [x] scout 打分 v2（0.32，靶子是票池规模化后 Top 全被速刷户霸榜的实证）：ROI/Wilson 下界/可跟性（持仓时长+极端价位）/一致性（一把梭识别）替代绝对盈亏/裸胜率/规模分；排除收紧（胜率上限闸样本 50→15、套利指纹 20→10、新增持仓中位 <1h 速刷排除）；风控新增单目标单市场敞口闸
