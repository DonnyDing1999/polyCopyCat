...
- **排除**（宁严勿松）：样本不足、成交额太小、多日不活跃、回放亏损、胜率过低、频率过高（疑似机器人）、**大样本下胜率高得离谱（疑似结构性套利）**、**持仓浮亏占成本过高（疑似囤死仓/只认盈不认亏）**、**跨场馆套利单腿（胜率异常高 + 几乎全在贴近1.0平仓、几乎不割肉——输腿在别的账户/场外博彩，本钱包只见幸存赢腿，招聘版按盈亏/死仓看不穿）**，以及最关键的——**快进快出占比过高（疑似做市/套利）**。这类地址账面常年"盈利"、胜率漂亮，但赚的是价差返佣，方向毫无参考价值，跟单基本必亏
...
tests/                # 254 个单测，全部离线（HTTP/WS 均为注入的假实现）
...
版本号在 `pyproject.toml` 与 `polycopycat/__init__.py`（当前 0.27.0），每交付一个里程碑 minor +1。
